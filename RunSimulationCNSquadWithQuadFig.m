function RunSimulationCNSquadWithQuadFig(P)
%% Initialization
% Xtrue=zeros(12,P.Nq);
...
for i=1:iter
   ...
   % plotquadrotor2(states,P2);
    %% Real vehicle
    h=plot3DTraj(h,reshape(100*Xfilter,9,P.Nq),100*Xtrue,P);
    ...
    for qcount=1:P.Nq
        ...
        h2(qcount)=drawlines(100*xl,100*yl,100*zl,h2(qcount));
...
%     [Igps,igps]=AddGPS(1,Xtrue,P,Xfilter);
%     Yfilter=Yfilter+Igps;
%     yfilter=yfilter+igps;
%      [Igps,igps]=AddGPS(3,Xtrue,P,Xfilter);
%     Yfilter=Yfilter+Igps;
%     yfilter=yfilter+igps;
...
for j=1:9
subplot(3,3,j)
plot(T,Xtrue_plot(j,:,i)-Xfilter_plot(9*(i-1)+j,:),'r',T,3*Pfilter_plot(9*(i-1)+j,:),'b--',T,-3*Pfilter_plot(9*(i-1)+j,:),'b--')
xlabel('time(s)');
if(j==1)
ylabel('x_{err}(m)');
end
