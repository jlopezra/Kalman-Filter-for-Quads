function [Yt,yt]=TotalMeasurementInfoFromLM(Z,Xfilter,Xfilter0,P,index)
...
if(isempty(Z)==0)
    for i=1:size(Z,1)
        ...
        Xj=[P.Xl(j);P.Yl(j);P.Zl(j)];
        %% Info From range
%         Hr=SingleRangeJacobianLM(Xfilter0,P,j,index);
...
%         yt=yt+infoR;
        %% info from bearing 1
        [Hb1,Hb2]=SingleBearingJacobianLM(Xfilter0,P,j,index);
